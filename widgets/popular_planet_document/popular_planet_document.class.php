<?php
    /**
     * @class popluar_planet_document 
     * @author zero (zero@nzeo.com)
     * @brief 플래닛 인기 글 목록 출력
     * @version 0.1
     **/

    class popular_planet_document extends WidgetHandler {
        ...
        function proc($args) {
			...
			// 정렬 대상
			$args->sort_index = $args->sort_index;

            // 중복 허용/ 비허용 체크
            if($args->allow_repetition != 'Y') {
                $output = executeQueryArray('widgets.popular_planet_document.getUniquePopularDocuments', $args);
            } else {
                $output = executeQueryArray('widgets.popular_planet_document.getPopularDocuments', $args);
            }
